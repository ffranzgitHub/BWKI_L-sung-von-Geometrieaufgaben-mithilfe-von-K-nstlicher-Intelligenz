...
def __klassenanzahl_in_split(sorted_data:dict):
    ...
    for aufgabentyp, aufgaben in sorted_data.items():
        ...
        train_n_classes[aufgabentyp] = round(globale_variablen["split_train"]*anzahl_aufgaben)
    ...
    for aufgabentyp, aufgaben in sorted_data.items():
        anzahl_aufgaben = len(aufgaben)
        val_n_classes[aufgabentyp] = round(globale_variablen["split_val"]*anzahl_aufgaben)
...
def __add_split(data):
    ...
    sorted_data = dict.fromkeys(unique([d.get("Aufgabentyp") for d in data]), None) # sorted_data erstellen mit {"Aufgabentpy1": ["text1", "text2"], "Aufgabentyp2", [text3, text4]}
